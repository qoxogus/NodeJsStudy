...
exports.update_process = function(request, response){
    var body = '';
      ...
      request.on('end', function(){
          var post = qs.parse(body);
          db.query(`
          UPDATE author SET name=?, profile=? WHERE id=?`, //상세보기를 만들 때 db 테이블에 인서트를 해준다 
...
exports.delete_process = function(request, response){
    var body = '';
      ...
      request.on('end', function(){
          var post = qs.parse(body);
          db.query(`DELETE FROM topic WHERE author_id=?`, //저자를 지우기 전 저자가 쓴 글도 삭제하게 만든다. 즉 저자를 지우면 저자가 쓴 글도 삭제하게 하는 것.
          [post.id],
          function(error1, result1){
              ...
              db.query(`
                DELETE FROM author WHERE id=?`, //상세보기를 만들 때 db 테이블에 인서트를 해준다 
                [post.id],
                function(error, result){
                    if(error){
                        throw error;
