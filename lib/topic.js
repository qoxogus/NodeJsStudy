...
exports.page = function(request, response){
    ...
    db.query(`SELECT * FROM topic`, function(error, topics) {
        ...
        var query = db.query(`SELECT * FROM topic LEFT JOIN author ON topic.author_id=author.id WHERE topic.id=?`, [queryData.id], function(error2, topic){ //보안 문제 때문에 id값에 '?'를 주고 2번째 인자값으로 []안에 '?'에 들어갈 무언가를 넣어준다.   id=${queryData.id} == [queryData.id],  링크를 클릭했을 때 sql문을 보기 위해 query변수 추가
          if(error2){
            throw error2;
          }
        ...
        //  console.log(topic);
         ...
         console.log(query.sql); //query변수 sql문 출력
         response.writeHead(200); //서버와의 약속 404이면 찾을수없다는 약속이다.
         response.end(html); //화면에 보여지는 것
        })
     });
